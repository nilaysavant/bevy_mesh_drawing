...
pub fn handle_vertex_indicator_grab(
    ...
    let DrawingMode::EditMode(EditModeState {active_vertex_indicator: Some(active_vertex_indicator), ..}) = drawing_state.mode else { return; };
    let Ok(canvas_transform) = query_canvas.get_single() else { return; };
    let Ok(mut transform) = query_indicators.get_mut(active_vertex_indicator) else { return; };
